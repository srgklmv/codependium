...
**defer** – ключевое слово, которое позволяет отложить выполнение функции. Аргументы
функции вычисляются немедленно (прямо в строке с `defer`). Можно также пользоваться 
замыканиями.
...
**Полезные ссылки:**
- [Каталог статей от ИТМО о базах данных](https://neerc.ifmo.ru/wiki/index.php?title=%D0%91%D0%B0%D0%B7%D1%8B_%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85)
...
- [ ] Расписать подробнее про аномалии.

**Нормализация** – это процесс пересмотра таблиц и связей в реляционной БД с целью
уменьшения количества дублируемых данных и аномалий, связанных с ними.
...
- [ ] Дописать про Бойса-Кодда и другие нормальные формы. 

**Первая нормальная форма**

Таблица соответствует 1НФ, если не имеет дублирующихся строк, а значения во всех 
столбцах являются атомарными (без перечислений или составных типов данных).
...
Отношение соответствует 2НФ, если:
- Отношение соответствует 1НФ;
- Все атрибуты, не являющиеся ключом, _функционально зависят_ от всего ключа, а не от его части.

По сути, это значит, что если в отношении есть первичный ключ, то атрибуты отношения
не должны содержать значений, которые зависят от части этого первичного ключа. 
Одновременно с этим отношение вполне может обладать транзитивными зависимостями.

Не соответствует 2НФ:

```text
+--------+--------+------+--------+
| Марка  | Модель | Цена | Скидка |
+--------+--------+------+--------+
| BMW    | M5     | 5000 | 5%     |
+--------+--------+------+--------+
| Nissan | GT-R   | 5500 | 10%    |
...
Здесь первичным ключом является связка аттрибутов "Марка" и "Модель", от которой 
напрямую зависит цена. Но скидка зависит только от аттрибута "Марка", поэтому она
должна быть вынесена в отдельную таблицу.
...
Отношение соответствует 3НФ, если:
- Отношение соответствует 2НФ;
- Все атрибуты, зависят от ключа напрямую (нетранзитивно).

Не соответствует 3НФ:

```text
+--------+--------+--------+--------+
| Марка  | Модель | Страна | Руль   |
+--------+--------+--------+--------+
| BMW    | M5     | Россия | Левый  |
+--------+--------+--------+--------+
| Nissan | GT-R   | Англия | Правый |
+--------+--------+--------+--------+
| BMW    | M1     | Россия | Левый  |
+--------+--------+--------+--------+
| BMW    | X5M    | Англия | Правый |
+--------+--------+--------+--------+
```
